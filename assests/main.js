...
for (var i = 0 ;i< musics.length; i++) {
    playList.insertAdjacentHTML( 'beforeend',
       `<div class="playlist playlist--hover ${i === indexSong ? 'active' : ''}" data-index=${musics[i].id}>
            <p class="item__number">${i === indexSong? '<i class="fas fa-volume-up"></i>' : `${musics[i].number}`}</p>
            <p class="item__title">${musics[i].title}</p>
            <p class="item__artist">${musics[i].artist}</p>
            <p class="item__time">${musics[i].time}</p>
        </div>`)
}
...
function resetSong(dir) {
    dir = Number (dir);
    playList.innerHTML =`
    <div class="playlist playlist-list__item">
        <p class="item__number">#</p>
        <p class="item__title">Tilte</p>
        <p class="item__artist">Artist</p>
        <p class="item__time">Time</p>
    </div>`;
    for (var j = 0 ;j< musics.length; j++) {
        playList.insertAdjacentHTML( 'beforeend',
       `<div class="playlist playlist--hover ${j === dir ? 'active' : ''}" data-index=${musics[j].id}>
            <p class="item__number">${j === dir? '<i class="fas fa-volume-up"></i>' : `${musics[j].number}`}</p>
            <p class="item__title">${musics[j].title}</p>
            <p class="item__artist">${musics[j].artist}</p>
            <p class="item__time">${musics[j].time}</p>
        </div>`)
    }
}
...
//       DARK THEME
const ball = document.querySelector('.header-check__ball');
const blackThemeBtn = document.querySelector('.header-check');
...
const hideNavBar = document.querySelector('.mb-navbar');
const hideSideBar = document.querySelector('.mb-sidebar');
